...
GSS_QUESTIONS_COMPREHENSIVE = GSS_QUESTIONS
...
def main():
    ...
    persona_files = [
    PROJECT_DIR / "data" / "personas_demographics_political.csv", # /home/sant6886/ArtSoc/ArtificialSocieties/personas_demographics_political.csv
    PROJECT_DIR / "data" / "personas_demographics.csv", # /home/sant6886/ArtSoc/ArtificialSocieties/personas_demographics.csv
    PROJECT_DIR / "data" / "gss2024_personas.csv", # /home/sant6886/ArtSoc/ArtificialSocieties/generation/data/gss2024_personas.csv
    ]

    # Filter to only existing files
    existing_persona_files = [f for f in persona_files if f.exists()]

    if not existing_persona_files:
        raise FileNotFoundError(
            f"No persona files found for year {year}. "
            f"Looked for: {[str(f) for f in persona_files]}"
        )

    print(f"Found {len(existing_persona_files)} persona file(s):")
    for pf in existing_persona_files:
        print(f"  - {pf.name}")
    print()

    # --- Iterate over each persona file ---
    for personas_file in existing_persona_files:
        ...
        # Derive a tag from the filename for output separation
        file_tag = personas_file.stem  # e.g. "gss2024_personas_2"

        print("=" * 70)
        print(f"Persona file: {personas_file.name}  ({len(personas_df)} personas)")
        print("=" * 70)
        print()

        total_requests = len(models) * len(personas_df) * len(GSS_QUESTIONS_COMPREHENSIVE) * args.runs
        print(f"Total requests for this file: {total_requests:,}")
        print()

        # Process each model
        for model in models:
            print("-" * 70)
            print(f"Processing: {model}  |  Personas: {file_tag}")
            print("-" * 70)

            model_filename = model.replace("/", "_")
            output_file = output_dir / f"{model_filename}_{file_tag}.csv"

            completed_tasks = load_completed_tasks(output_file)

            if completed_tasks:
                print(f"Found {len(completed_tasks)} already-completed tasks — resuming")
            ...
            if not tasks:
                print(f"All tasks already completed for {model} / {file_tag}")
                print(f"Output file: {output_file}\n")
                continue

            print(f"Tasks: {len(tasks)} remaining, {len(completed_tasks)} done ({total_tasks} total)\n")
            ...
            with ThreadPoolExecutor(max_workers=args.max_workers) as executor:
                ...
                pbar = tqdm(total=len(tasks), desc=f"{model.split('/')[-1]} [{file_tag}]")
    ...
    print("=" * 70)
    print("All models and persona files complete!")
    print("=" * 70)
